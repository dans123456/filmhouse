...
const CSV_FILE_PATH = "./Data/datafile.csv";
const JSON_FILE_PATH = "./Data/movies_metadata.json";
...
async function initializeDatabase() {
    ...
    if (cachedData) {
        try {
            ...
            // If the cache was generated at the root, strip MOVIE/ from local assets
            parsed.forEach(m => {
                if (m.poster && m.poster.startsWith("MOVIE/img/")) {
                    m.poster = m.poster.substring(6); // strip "MOVIE/"
                }
                if (m.backdrop && m.backdrop.startsWith("MOVIE/img/")) {
                    m.backdrop = m.backdrop.substring(6); // strip "MOVIE/"
                }
            });
            state.movies = parsed;
